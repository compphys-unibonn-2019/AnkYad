void RW2()
{
 ...
 double S[iter], R[iter], avg=0, sd[iter],d[100];
...
//
  ...
  d[j]=TMath::Sqrt(((x[N-1]-x[0])*(x[N-1]-x[0]))+((y[N-1]-y[0])*(y[N-1]-y[0])));
  ...
  for (int j=0; j<100; j++)
  {
	d[j]=TMath::Abs(d[j]-R[k]);
	avg=avg+(d[j]*d[j]);
  }
  sd[k]=sqrt(avg/100);
  cout << "steps  " << S[k] << "  Range  " << R[k] << "  SD  " << sd[k] << endl ;
...
//
 ...
 TGraphErrors* hdist = new TGraphErrors(iter,S,R,nullptr,sd);
 TF1* hN = new TF1("hN","[0]+[1]*x",30,100);
 for (int k=0; k<=90; k++)
 {
  //hdist->Fill(S[k],R[k]);
